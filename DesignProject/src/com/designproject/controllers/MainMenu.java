...
public class MainMenu extends NavigationDrawerActivity {
	...
	public void inspectionClickListener(View view) {
		...
		Intent openInspectionView = new Intent(MainMenu.this,
	...
	private void calculateDates() {
		...
		for (Client client : mFranchise.getClients()) {
			...
			for (Contract contract : client.getContracts()) {
				...
				} else if (nextInspectionDate.isBefore(DateTime.now()
						.plusMonths(1))) {
					dueNextMonth++;
		...
		updateProgressBars(dueToday, doneToday, dueNextWeek, doneNextWeek,
				dueNextMonth, doneNextMonth, totalContractsCompleted);
	}

	private void updateProgressBars(int dueToday, int doneToday,
			int dueNextWeek, int doneNextWeek, int dueNextMonth,
			int doneNextMonth, int totalContractsCompleted) {
