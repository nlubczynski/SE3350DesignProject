...
public class FloorController extends NavigationDrawerActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (Room inspectionInformation : rooms) {
			...
			item.put("line3",
					String.valueOf(inspectionInformation.getEquipment().length));
			listInformationString.add(item);
		}

		// Create a list view displaying each room with its corresponding
		// information
		SimpleAdapter simpleAdapter = new SimpleAdapter(this,
				listInformationString, R.layout.room_list_item, new String[] {
						"line1", "line2", "line3" }, new int[] {
						R.id.RoomNameValue, R.id.RoomIDValue,
		...
		if (myList.getChildCount() == rooms.length) {
			...
			for (int i = 0; i < rooms.length; i++) {
				...
				if (rooms[i].isCompleted()) {
					View toBeColored = (View) ((ViewGroup) listElement)
							.getChildAt(0);
					TextView textToBeColored1 = (TextView) ((ViewGroup) toBeColored)
							.getChildAt(0);
					TextView textToBeColored2 = (TextView) ((ViewGroup) toBeColored)
							.getChildAt(1);
					textToBeColored1.setTextColor(getResources().getColor(
							R.color.green));
					textToBeColored2.setTextColor(getResources().getColor(
							R.color.green));
				}
				...
				View toBeUpdated = (View) ((ViewGroup) listElement)
						.getChildAt(2);
				TextView textToChange = (TextView) ((ViewGroup) toBeUpdated)
						.getChildAt(1);
